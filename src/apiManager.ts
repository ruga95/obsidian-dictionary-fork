...
export default class APIManager {
  ...
  public async requestSynonyms(query: string, pos?: PartOfSpeech): Promise<Synonym[]> {
    ...
    console.log("requestSynonyms api :", api);
    ...
    const { cache, settings } = this.plugin;
    if (settings.useCaching && !api.name.toLowerCase().contains("offline")) {
      const cachedSynonymCollection = cache.cachedSynonyms.find(
        (s) => s.word.toLowerCase() == query.toLowerCase() && s.lang == settings.defaultLanguage && s.api == api.name
      );
