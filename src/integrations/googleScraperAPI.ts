...
export class GoogleScraperDefinitionProvider extends Base implements DefinitionProvider {
  async requestDefinitions(query: string, lang: string): Promise<DictionaryWord> {
    const url = `https://www.google.com/search?q=define+${query.replace(/\s/g, '+')}+${GoogleScraperDefinitionProvider.LANGUAGES[lang]}`;
    const headers = { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36' };

    console.log("GoogleScraperDefinitionProvider requestDefinitions result :", url);
...
export class GoogleScraperSynonymProvider extends Base implements SynonymProvider {
  ...
  async requestSynonyms(query: string, lang: string, _?: PartOfSpeech): Promise<Synonym[]> {
    ...
    console.log("GoogleScraperSynonymProvider requestSynonyms :", query, lang, _);

    return (await this.provider.requestDefinitions(query, lang))
      .meanings.first()
      .definitions.first()
      .synonyms.map<Synonym>(synonym => { return { word: synonym } });
  }
}