...
abstract class Base {
  ...
  protected constructRequest(query: string, lang: string): string {
    const url = this.API_END_POINT + lang + "/" + query;
    console.log("constructRequest url :", url);
    return url;
    //SCHEMA: https://api.dictionaryapi.dev/api/v2/entries/<language_code>/<word>
  }
}

export class FreeDictionaryDefinitionProvider
  extends Base
...
export class FreeDictionarySynonymProvider
  extends Base
  implements SynonymProvider {
  public supportedLanguages: string[] = ["en_US"];
  ...
  async requestSynonyms(query: string, lang: string, pos?: PartOfSpeech): Promise<Synonym[]> {
    ...
    try {
      result = await request({
        url: this.constructRequest(query, this.languageCodes[lang]),
      });
    } catch (error) {
      return Promise.reject(error);
    }
    ...
    const meanings: Meaning[] = (
      (await JSON.parse(result)) as DictionaryWord[]
    ).first().meanings;
    ...
    console.log("requestSynonyms nonPOSMatch + synonyms :", nonPOSMatch, synonyms);
